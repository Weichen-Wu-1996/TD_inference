...
def TD_iid(MRP: Markov_Reward_Process,
        ...
        if t in save_iter:
            ...
            if estimate_variance:
                ...
                for j in range(N_trials):
                    A_bar_inv = np.linalg.inv(A_bars[j] + 1e-8 * np.identity(MRP.d))
                    Lambda_hat = A_bar_inv @ Gamma_hats[j] @ A_bar_inv.T
    ...
    results = {'T': T,
               ...
               'alpha': alpha,
               'saved_delta_bars': saved_theta_bars - MRP.theta_star.reshape(1,1,-1)}

    if save_original:
        results['saved_deltas'] = saved_thetas - MRP.theta_star.reshape(1,1,-1)
